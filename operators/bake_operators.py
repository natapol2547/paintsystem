    ...
    def execute(self, context):
        ...
        if self.as_layer:
            ...
            if 'multi_object' in sig.parameters:
                active_channel.bake(context, mat, bake_image, self.uv_map_name, force_alpha=True, multi_object=self.multi_object)
            else:
                # Fallback for older runtime that hasn't reloaded Channel.bake yet
            ...
            sig = inspect.signature(active_channel.bake)
            if 'multi_object' in sig.parameters:
                active_channel.bake(context, mat, bake_image, self.uv_map_name, multi_object=self.multi_object)
            else:
