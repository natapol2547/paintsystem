...
class PAINTSYSTEM_OT_BoxBlur(PSContextMixin, PSImageFilterMixin, Operator):
    bl_idname = "paint_system.box_blur"
    bl_label = "Box Blur"
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = "Apply a Box blur to the active image"
    
    box_radius: FloatProperty(name="Box Radius", default=3.0, min=0.1, max=10.0, step=0.1)
    ...
    def execute(self, context):
        ...
        np_image = box_blur(np_image, self.box_radius)
...
class PAINTSYSTEM_OT_BrushPainter(PSContextMixin, PSImageFilterMixin, Operator):
    ...
    box_radius: IntProperty(name="Gaussian Sigma", default=3, min=0, max=10)
    ...
    def execute(self, context):
        ...
        painter.box_radius = self.box_radius
